...
const RenderComponentFromDevice: FC<{
  device: IDevice;
  componentUpdated: (device: IDevice) => void;
}> = ({ device, componentUpdated }) => {
  switch (device.type) {
    case "toggle":
      return (
        <ToggleDevice
          ...
          onReceiveUpdate={(d) => componentUpdated(d)}
        />
      );
    case "fan":
      return (
    ...
    case "buzzer":
      return (
        <SpeakerDevice
          device={device}
          playIcon={faPlay}
          noTuneSelectedIcon={faStopCircle}
          onReceiveUpdate={(d) => componentUpdated(d)}
        />
      );
    case "screen":
