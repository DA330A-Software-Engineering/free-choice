...
const PrivateRouteWrapper: FC<{ restricted?: boolean; redirectTo: string }> = ({
  ...
  const auth = useAuth();
  return (auth.getToken() !== null) === restricted ? (
...
const Header: FC = () => {
  ...
  if (auth.getToken()) {
    return (
      <div className="header-container">
        <Link style={{ opacity: opacityFromPath("/") }} to={"/"}>
          Devices
        </Link>
        <Link style={{ opacity: opacityFromPath("/groups") }} to={"/groups"}>
          Groups
        </Link>
        <Link
          style={{ opacity: opacityFromPath("/routines") }}
          to={"/routines"}
        >
          Routines
        </Link>
        <Link
          style={{ opacity: opacityFromPath("/createtrigger") }}
          to={"/createtrigger"}
        >
          Create Trigger
        </Link>
        <a onClick={() => auth.logout(() => null)} id="logout">
          Logout
        </a>
      </div>
    );
  ...
  return (
    <div className="header-container">
      <Link style={{ opacity: opacityFromPath("/login") }} to={"/login"}>
        Login
      </Link>
      <Link style={{ opacity: opacityFromPath("/signup") }} to={"/signup"}>
        Signup
      </Link>
    </div>
  );
      ...
      <DeviceContextProvider>
        <BrowserRouter>
          ...
          <Routes>
            ...
            <Route
              path="/"
              element={<PrivateRouteWrapper redirectTo="/login" />}
            >
            ...
            <Route
              path="/"
              element={<PrivateRouteWrapper restricted={true} redirectTo="/" />}
            >
